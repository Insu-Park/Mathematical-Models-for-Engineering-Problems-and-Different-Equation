...
function [ys] = Heun_2nd(f, a, b, hs, y0)
	for j=1:length(hs)
		...
		for i=1:(n-1)
			slopeK1 = f(x(i),y(i));
			slopeK2 = f(x(i)+(2/3*h),y(i)+(2/3*slopeK1*h));
			y(i+1) = y(i) + ((slopeK1/4)+(slopeK2*3/4))*h;
		end
		ys(j) = y(n);
	end		
end
...
function [ys] = RK3_classical(f, a, b, hs, y0)
	for j=1:length(hs)
		...
		for i=1:(n-1)
			slopeK1 = f(x(i),y(i));
			slopeK2 = f(x(i)+(1/2*h),y(i)+(1/2*slopeK1*h));
			slopeK3 = f(x(i)+h,y(i)-(slopeK1*h)+(2*slopeK2*h));
			y(i+1) = y(i) + ((slopeK1/6)+(slopeK2*2/3)+(slopeK3/6))*h;
		end
		ys(j) = y(n);
	end		
end
...
function [ys] = RK3_Nystrom(f, a, b, hs, y0)
	for j=1:length(hs)
		...
		for i=1:(n-1)
			slopeK1 = f(x(i),y(i));
			slopeK2 = f(x(i)+(2/3*h),y(i)+(2/3*slopeK1*h));
			slopeK3 = f(x(i)+(2/3*h),y(i)+(2/3*slopeK2*h));
			y(i+1) = y(i) + ((slopeK1/4)+(slopeK2*3/8)+(slopeK3*3/8))*h;
		end
		ys(j) = y(n);
	end		
end
...
function [ys] = RK4_classical(f, a, b, hs, y0)
	for j=1:length(hs)
		...
		for i=1:(n-1)
			slopeK1 = f(x(i),y(i));
			slopeK2 = f(x(i)+(1/2*h),y(i)+(1/2*slopeK1*h));
			slopeK3 = f(x(i)+(1/2*h),y(i)+(1/2*slopeK2*h));
			slopeK4 = f(x(i)+h,y(i)+(slopeK3*h));
			y(i+1) = y(i) + ((slopeK1/6)+(slopeK2/3)+(slopeK3/3)+(slopeK4/6))*h;
		end
		ys(j) = y(n);
	end		
end
